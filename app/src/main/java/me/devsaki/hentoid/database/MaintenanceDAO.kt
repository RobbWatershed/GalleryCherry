...
class MaintenanceDAO {
    ...
    fun selectDownloadedNHBooksIncompleteUrls(): Set<Long> {
        val okayIds = store.boxFor(Content::class.java).query()
            .equal(Content_.site, Site.NHENTAI.code.toLong())
    ...
    fun selectDownloadedHiperdexBooksObsoleteUrls(): Set<Long> {
        return store.boxFor(Content::class.java).query()
            .equal(Content_.site, Site.HIPERDEX.code.toLong())
            .startsWith(Content_.dbUrl, "manhwa/", QueryBuilder.StringOrder.CASE_INSENSITIVE)
            .safeFindIds().toSet()
    }
