...
class PawParser : BaseImageListParser() {
    ...
    private fun getPages(onlineContent: Content): List<ImageFile> {
        try {
            val useMobileAgent = Site.PAWCHIVE.useMobileAgent
            val useHentoidAgent = Site.PAWCHIVE.useHentoidAgent
            val useWebviewAgent = Site.PAWCHIVE.useWebviewAgent
            ...
            val parts = PawParts(onlineContent.url)

            setRateLimit(Site.PAWCHIVE.requestsCapPerSecond.toLong())
